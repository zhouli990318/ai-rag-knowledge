...
import { ui } from '../theme/semanticColors';
...
// Allow KaTeX-specific attributes through sanitizer
const sanitizeSchema = {
  ...
  tagNames: [...(defaultSchema.tagNames || []), 'math', 'semantics', 'mrow', 'mi', 'mo', 'mn', 'msup', 'msub', 'mfrac', 'mtext', 'annotation'],
};
const REHYPE_PLUGINS = [[rehypeSanitize, sanitizeSchema], rehypeKatex] as any[];
...
export default memo(function MarkdownRenderer({ content, isStreaming }: Props) {
  ...
  return (
    <Box sx={{
      ...
      '& li::marker': { color: ui.accentRed },
      /* Links */
      '& a': { color: ui.accentRed, textDecoration: 'none', transition: 'all 150ms', '&:hover': { textDecoration: 'underline' } },
      /* Blockquotes - 水墨竖线风格 */
      '& blockquote': {
        borderLeft: `3px solid ${ui.accentRed}`,
        pl: 1.5, ml: 0, my: 0.85,
        py: 0.5, pr: 1,
        color: '#6A6660',
      ...
      /* Strong/Bold */
      '& strong': { fontWeight: 700, color: ui.textPrimary },
      /* HR */
      '& hr': {
        border: 'none',
        height: 1,
