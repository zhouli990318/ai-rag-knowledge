  ...
  return (
    <Box
      sx={{
        ...
        border: `1px solid ${focused ? `${ink.gray}40` : 'rgba(224,221,216,0.6)'}`,
    ...
    >
      <SearchIcon sx={{ fontSize: 19, color: focused ? ink.gray : ink.muted, flexShrink: 0 }} />
      <InputBase
        fullWidth
        value={value}
      ...
      {value && (
        <IconButton size="small" onClick={() => onChange('')} sx={{ p: 0.3 }}>
          <Box component="span" sx={{ fontSize: 16, color: ink.muted, lineHeight: 1 }}>×</Box>
        </IconButton>
      )}
    </Box>
  );
